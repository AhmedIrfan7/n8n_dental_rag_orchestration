  ...
  writeWf(opts.fileName, {
    ...
    nodes: [
      ...
      { parameters: {
        ...
        retryOnFail: true, maxTries: 3, waitBetweenTries: 600,
        credentials: { openAiApi: { id: '__OPENAI_CRED_ID__', name: 'OpenAI Dental' } } },
...
writeWf('10_orchestrator.json', {
  ...
  nodes: [
    ...
    // A plain opening greeting ("hi", "thanks") needs zero LLM calls to
    // answer correctly (see classify_intent.js's is_greeting) - this branch
    // skips the classify network round trip entirely instead of paying for
    // it just to learn the answer is "no real intent". Both branches below
    // converge back into RouteAndCall, which reads its intents from
    // $input (not a named node reference), so it works unchanged either way.
    { parameters: { conditions: { options: { caseSensitive: true, leftValue: '', typeValidation: 'strict' },
        conditions: [{ id: 'g1', leftValue: '={{ $json.is_greeting }}', rightValue: true, operator: { type: 'boolean', operation: 'true', singleValue: true } }], combinator: 'and' } },
      id: 'a0000000-0000-0000-0000-00000000000g', name: 'IsGreeting', type: 'n8n-nodes-base.if', typeVersion: 2.2, position: [20, 0] },
    { parameters: { jsCode: "return [{ json: { choices: [{ message: { content: JSON.stringify({ intents: [] }) } }] } }];" },
      id: 'a0000000-0000-0000-0000-00000000000h', name: 'ClassifySkipOpenAI', type: 'n8n-nodes-base.code', typeVersion: 2, position: [20, 140] },
    { parameters: {
        method: 'POST', url: 'https://api.openai.com/v1/chat/completions',
        authentication: 'predefinedCredentialType', nodeCredentialType: 'openAiApi',
        sendBody: true, specifyBody: 'json', jsonBody: '={{ JSON.stringify($json.oai_body) }}',
      ...
      id: 'a0000000-0000-0000-0000-000000000003', name: 'ClassifyOpenAI', type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [20, -140],
      retryOnFail: true, maxTries: 3, waitBetweenTries: 600,
      credentials: { openAiApi: { id: '__OPENAI_CRED_ID__', name: 'OpenAI Dental' } } },
  ...
  connections: {
    ...
    ClassifyIntent: { main: [[{ node: 'IsGreeting', type: 'main', index: 0 }]] },
    IsGreeting: { main: [[{ node: 'ClassifySkipOpenAI', type: 'main', index: 0 }], [{ node: 'ClassifyOpenAI', type: 'main', index: 0 }]] },
    ClassifySkipOpenAI: { main: [[{ node: 'RouteAndCall', type: 'main', index: 0 }]] },
